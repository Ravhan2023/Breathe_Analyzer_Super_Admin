...
const Admin = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`${baseurl}/api/department-admin/`, {
        headers: {
  ...
  const uniquDepartmentNames = [
    "All Departments",
    ...new Set(data.map((company) => company.Department_name)),
  ];
  ...
  const filteredData = data.filter((company) => {
    return (
      ...
      (selectedDepartment === "All Departments" ||
        company.Department_name === selectedDepartment) &&
      company.admin_name.toLowerCase().includes(search.toLowerCase())
    );
  });
